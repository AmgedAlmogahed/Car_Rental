...
interface CarsDao{
    ...
    @Transaction
    @Query("SELECT customers.name, renting.price, renting.startDate, renting.endDate,renting.id FROM customers,renting WHERE customers.id = customer_id AND car_id = :key ORDER BY renting.endDate DESC")
